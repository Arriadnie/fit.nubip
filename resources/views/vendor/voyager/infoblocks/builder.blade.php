...
@section('page_title', 'Конструктор інфоблоків')

@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i>{{ 'Конструктор інфоблоків' }} ({{ $infoblock->name }})
        <div class="btn btn-success add_item"><i class="voyager-plus"></i> {{ 'Новий елемент інфоблоку' }}</div>
    </h1>
...
@section('content')
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">{{ 'Перетягніть елементи інфоблоку нижче, щоб змінити їх порядок.' }}</p>
                    </div>

                    <div class="panel-body" style="padding:30px;">
                        <div class="dd">
                            @include('voyager::infoblocks.partial.items-list', [
                                'items' => $infoblock->items,
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('voyager.infoblocks.item.destroy', ['infoblock' => $infoblock->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
    ...
    <div class="modal modal-info fade" tabindex="-1" id="infoblock_item_modal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager::generic.close') }}"><span
                ...
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('voyager.infoblocks.item.add', ['infoblock' => $infoblock->id]) }}"
                      data-action-update="{{ route('voyager.infoblocks.item.update', ['infoblock' => $infoblock->id]) }}">
...
{{--                        </select>--}}
                        <input type="hidden" name="infoblock_id" value="{{ $infoblock->id }}">
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
             ...
             */
            $('#infoblock_item_modal').multilingual({
                "form":          'form',
                "transInputs":   '#infoblock_item_modal input[data-i18n=true]',
             ...
             * Set Variables
             */
            var $m_modal       = $('#infoblock_item_modal'),
            ...
            $('.item_actions').on('click', '.edit', function (e) {
                window.location.href = '{{ url('admin/infoblock-items') }}/' +
                    e.currentTarget.getAttribute('data-id') + '/edit';
                //$m_form.find("input[type=submit]").val('{{ __('voyager::generic.update') }}');
                //$m_modal.modal('show', {data: $(e.currentTarget)});
            });

            /**
             * infoblock Modal is Open
             */
            ...
            $('.item_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_form')[0].action = '{{ route('voyager.infoblocks.item.destroy', ['infoblock' => $infoblock->id, 'id' => '']) }}/' + id;
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('voyager.infoblocks.order',['infoblock' => $infoblock->id]) }}', {
